...
export const ChartDrawingLayer: React.FC<Props> = ({
  ...
  const [dragStartInfo, setDragStartInfo] = useState<{ price: number; time: number; initialDrawing: Drawing } | null>(null);
  ...
  const draw = useCallback(() => {
      ...
      if (d.type === 'long_position' || d.type === 'short_position') {
        ...
        if (xE != null && yE != null && yTP != null && ySL != null) {
          ...
          if (yOneToOne !== null) {
            ...
            ctx.beginPath(); ctx.moveTo(xE, yOneToOne); ctx.lineTo(xE + boxWidth, yOneToOne); ctx.stroke();
            ctx.setLineDash([]);
          }
          ...
          if ((d as PositionDrawing).isLocked) {
            ctx.save(); ctx.shadowBlur = 8; ctx.shadowColor = '#fcd535'; ctx.strokeStyle = '#fcd535'; ctx.fillStyle = '#fcd535';
            const lx = xE + boxWidth - 20; const ly = Math.min(yTP, yE) + 4;
            ctx.beginPath(); drawRoundRect(ctx, lx, ly + 6, 12, 9, 2); ctx.fill();
            ctx.beginPath(); ctx.arc(lx + 6, ly + 7, 4.5, Math.PI, 2 * Math.PI); ctx.stroke();
            ctx.restore();
          } else if (isSelected) {
            const gx = xE + boxWidth + 2; const gyCentre = (Math.min(yTP, yE) + Math.max(ySL, yE)) / 2;
            ctx.save(); ctx.fillStyle = 'rgba(255,255,255,0.45)'; ctx.shadowBlur = 5; ctx.shadowColor = 'rgba(255,255,255,0.6)';
            drawRoundRect(ctx, gx, gyCentre - 20, 4, 40, 2); ctx.fill();
            ctx.restore();
          }
        }
      }
    ...
    if (activeTool === 'trendline' && draftStart && mousePos) {
      ctx.save(); ctx.strokeStyle = TOOL_COLORS.trendline; ctx.lineWidth = 1.5; ctx.shadowBlur = 15; ctx.shadowColor = TOOL_COLORS.trendline;
      ctx.setLineDash([8, 6]); ctx.beginPath(); ctx.moveTo(draftStart.px, draftStart.py); ctx.lineTo(mousePos.x, mousePos.y); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = TOOL_COLORS.trendline; ctx.shadowBlur = 0;
      ...
      const draftPrice = xyToPrice(mousePos.y) || 0; const priceDelta = draftPrice - draftStart.price; const pctChange = (priceDelta / draftStart.price) * 100;
      const sign = priceDelta >= 0 ? '+' : ''; const text = `${sign}${priceDelta.toFixed(2)} (${sign}${pctChange.toFixed(2)}%)`;
      ctx.font = 'bold 10px monospace'; const tw = ctx.measureText(text).width;
      const cx = (draftStart.px + mousePos.x) / 2; const cy = (draftStart.py + mousePos.y) / 2;
      ctx.shadowBlur = 10; ctx.fillStyle = 'rgba(5, 11, 20, 0.95)'; ctx.beginPath(); drawRoundRect(ctx, cx - (tw/2) - 8, cy - 24, tw + 16, 18, 4); ctx.fill();
      ctx.shadowBlur = 0; ctx.fillStyle = priceDelta >= 0 ? '#00E5FF' : '#FF007F'; ctx.fillText(text, cx - (tw/2), cy - 12);
      ctx.restore();
    }

    animRef.current = requestAnimationFrame(draw);
  }, [drawings, selectedId, activeTool, draftStart, mousePos, priceToY, timeToX, interval]);
  ...
  const hitTest = useCallback((x: number, y: number) => {
    for (let i = drawings.length - 1; i >= 0; i--) {
      ...
      if (d.type === 'long_position' || d.type === 'short_position') {
        ...
        if (dx != null && yE != null && yTP != null && ySL != null) {
          ...
          if (Math.hypot(x - hX, y - yTP) < 12) return { id: d.id, part: 'tp' as const };
          if (Math.hypot(x - hX, y - ySL) < 12) return { id: d.id, part: 'sl' as const };
          if (Math.hypot(x - hX, y - yE) < 12) return { id: d.id, part: 'entry' as const };
          if (x >= dx && x <= dx + boxWidth) {
            if (Math.abs(y - yTP) < 8) return { id: d.id, part: 'tp' as const };
            if (Math.abs(y - ySL) < 8) return { id: d.id, part: 'sl' as const };
            if (Math.abs(y - yE) < 8) return { id: d.id, part: 'entry' as const };
          }
          const rEdgeX = dx + boxWidth;
          if (Math.abs(x - rEdgeX) < 15 && y >= Math.min(yTP, ySL) - 10 && y <= Math.max(yTP, ySL) + 10) return { id: d.id, part: 'right_edge' as const };
          if (x >= dx && x <= dx + boxWidth) {
            const minY = Math.min(yTP, ySL, yE); const maxY = Math.max(yTP, ySL, yE);
      ...
      if (d.type === 'trendline') {
        ...
        if (x1 != null && y1 != null && x2 != null && y2 != null) {
          ...
          const dx1 = x2 - x1; const dy1 = y2 - y1; const len2 = dx1 * dx1 + dy1 * dy1;
          if (len2 > 0) {
    ...
    return null;
  }, [drawings, priceToY, timeToX, interval]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (draggingId && draggingPart && dragStartInfo) {
      const price = xyToPrice(y);
      const time = xyToTime(x);
      if (price == null || time == null) return;
      const deltaPrice = price - dragStartInfo.price;
      const deltaTime = time - dragStartInfo.time;
      const initial = dragStartInfo.initialDrawing;

      setDrawings(prev => prev.map(d => {
        if (d.id !== draggingId) return d;
        if ((d as PositionDrawing).isLocked) return d;
        if (d.type === 'trendline' && initial.type === 'trendline') {
          if (draggingPart === 'p1') return { ...d, p1: { price, time } };
          if (draggingPart === 'p2') return { ...d, p2: { price, time } };
        }
        if ((d.type === 'long_position' || d.type === 'short_position') && (initial.type === 'long_position' || initial.type === 'short_position')) {
          if (draggingPart === 'entry') {
            return { ...d, entryPrice: initial.entryPrice + deltaPrice, entryTime: initial.entryTime + deltaTime, tpPrice: initial.tpPrice + deltaPrice, slPrice: initial.slPrice + deltaPrice, targetTime: initial.targetTime + deltaTime };
          }
          if (draggingPart === 'tp') return { ...d, tpPrice: initial.tpPrice + deltaPrice };
          if (draggingPart === 'sl') return { ...d, slPrice: initial.slPrice + deltaPrice };
          if (draggingPart === 'right_edge' || draggingPart === 'duration') {
            const newTargetTime = initial.targetTime + deltaTime;
            const newWidthInCandles = Math.round((newTargetTime - initial.entryTime) / (getIntervalMs(interval) / 1000));
            return { ...d, targetTime: newTargetTime, widthInCandles: Math.max(1, newWidthInCandles) };
          }
        }
        return d;
      }));
    }
  }, [draggingId, draggingPart, xyToPrice, xyToTime, dragStartInfo, interval]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (activeTool === 'none') {
      ...
      if (hit?.id && hit?.part) {
        ...
        if (drawing && price != null && time != null) {
          setDragStartInfo({ price, time, initialDrawing: JSON.parse(JSON.stringify(drawing)) });
        }
      }
      return;
    }
    ...
    if (activeTool === 'horizontal' && price != null) {
      const h: HorizontalDrawing = { id: uid(), type: 'horizontal', price, color: TOOL_COLORS.horizontal, width: 1.5, label: 'S/R', style: 'dashed' };
      setDrawings(prev => [...prev, h]); onToolChange('none'); return;
    }
    if (activeTool === 'trendline' && price != null && time != null) {
      if (!draftStart) setDraftStart({ px, py, price, time });
      else {
        const tl: TrendLineDrawing = { id: uid(), type: 'trendline', p1: { price: draftStart.price, time: draftStart.time }, p2: { price, time }, color: TOOL_COLORS.trendline, width: 1.5, extended: false };
        setDrawings(prev => [...prev, tl]); setDraftStart(null); onToolChange('none');
      }
      return;
    }
    if ((activeTool === 'long_position' || activeTool === 'short_position') && price != null && time != null) {
      let activeAtr = 0; const targetCandle = data.find(d => d.time === time) || data[data.length - 1];
      if (targetCandle?.atr) activeAtr = targetCandle.atr; else activeAtr = price * 0.01;
      const isLong = activeTool === 'long_position'; const slDist = activeAtr * 1.5; const tpDist = activeAtr * 3.0;
      const p: PositionDrawing = { id: uid(), type: activeTool, entryPrice: price, entryTime: time, slPrice: isLong ? price - slDist : price + slDist, tpPrice: isLong ? price + tpDist : price - tpDist, targetTime: time + (getIntervalMs(interval) * 10 / 1000), widthInCandles: 10, riskAmount: 100, isLocked: false };
      setDrawings(prev => [...prev, p]); onToolChange('none'); return;
    }
    if (activeTool === 'annotation' && price != null && time != null) {
      setAnnotationDraft({ price, time }); setAnnotationText(''); return;
    }
  }, [activeTool, draftStart, hitTest, xyToPrice, xyToTime, onToolChange, data, drawings, interval]);
  ...
  useEffect(() => {
    const keyHandler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { setDraftStart(null); setSelectedId(null); onToolChange('none'); }
    };
    const upHandler = () => { setDraggingId(null); setDraggingPart(null); setDragStartInfo(null); };
    const moveHandler = (e: MouseEvent) => {
      if (activeTool !== 'none' || draggingId) { setIsHoveringShape(true); return; }
      const rect = canvasRef.current?.getBoundingClientRect();
      if (!rect) return;
      const x = e.clientX - rect.left; const y = e.clientY - rect.top;
      const hit = hitTest(x, y); setIsHoveringShape(!!hit);
    };
    window.addEventListener('keydown', keyHandler);
    window.addEventListener('mouseup', upHandler);
    window.addEventListener('mousemove', moveHandler);
    return () => { window.removeEventListener('keydown', keyHandler); window.removeEventListener('mouseup', upHandler); window.removeEventListener('mousemove', moveHandler); };
  ...
  const confirmAnnotation = () => {
    ...
    const ann: AnnotationDrawing = { id: uid(), type: 'annotation', price: annotationDraft.price, time: annotationDraft.time, text: annotationText.trim(), color: TOOL_COLORS.annotation };
    setDrawings(prev => [...prev, ann]); setAnnotationDraft(null); setAnnotationText(''); onToolChange('none');
  };

  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    if (activeTool === 'none') return;
    const touch = e.touches[0];
    const mockEvent = { button: 0, clientX: touch.clientX, clientY: touch.clientY, preventDefault: () => e.preventDefault() } as unknown as React.MouseEvent;
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    if (!rect) return; setMousePos({ x: touch.clientX - rect.left, y: touch.clientY - rect.top });
  }, [activeTool, draftStart]);

  const handleTouchEnd = useCallback(() => {}, []);

  const cursor = draggingId ? 'grabbing' : draggingPart === 'right_edge' ? 'ew-resize' : activeTool === 'none' ? (() => {
    const hit = hitTest(mousePos?.x ?? 0, mousePos?.y ?? 0);
    if (!hit) return 'default'; if (hit.part === 'right_edge') return 'ew-resize'; return 'pointer';
  })() : activeTool === 'horizontal' ? 'row-resize' : activeTool === 'trendline' ? 'crosshair' : (activeTool === 'long_position' || activeTool === 'short_position') ? 'crosshair' : activeTool === 'annotation' ? 'cell' : 'default';
  ...
  return (
    <>
      <canvas
        ...
        style={{ cursor, pointerEvents: (activeTool !== 'none' || draggingId || isHoveringShape) ? 'auto' : 'none' }}
      ...
      {annotationDraft && (
        <div className="absolute z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 glass-tooltip p-4 w-72 font-sans overflow-hidden">
          ...
          <input autoFocus type="text" value={annotationText} onChange={e => setAnnotationText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') confirmAnnotation(); if (e.key === 'Escape') setAnnotationDraft(null); }} placeholder="Type transmission..." className="w-full bg-[#050B14]/80 border border-white/10 focus:border-[var(--holo-magenta)] rounded-md px-3 py-2 text-[#00E5FF] text-sm outline-none font-mono font-bold mb-3 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] relative z-10 transition-colors" />
      ...
      {selectedId && (
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 glass-panel rounded-full px-4 py-2 font-sans shadow-[0_10px_30px_rgba(0,0,0,0.8)] border border-[var(--holo-cyan)]/20">
          ...
          {(drawings.find(d => d.id === selectedId)?.type === 'long_position' || drawings.find(d => d.id === selectedId)?.type === 'short_position') && (() => {
            ...
            return (
              <button
                onClick={() => setDrawings(prev => prev.map(d => d.id === selectedId && (d.type === 'long_position' || d.type === 'short_position') ? { ...d, isLocked: !(d as PositionDrawing).isLocked } : d))}
                className={`flex items-center gap-1.5 border text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full transition-all hover-lift ${pos?.isLocked ? 'bg-[var(--holo-gold)]/20 border-[var(--holo-gold)]/60 text-[var(--holo-gold)] shadow-[0_0_8px_rgba(252,213,53,0.4)]' : 'bg-white/5 hover:bg-white/10 border-white/10 text-white/60'}`}
                title={pos?.isLocked ? 'Unlock' : 'Lock'}
              >
                {pos?.isLocked ? <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> : <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>}
                {pos?.isLocked ? 'Locked' : 'Lock'}
              </button>
            );
          })()}
          {drawings.find(d => d.id === selectedId)?.type === 'trendline' && (
            <button onClick={() => setDrawings(prev => prev.map(d => d.id === selectedId && d.type === 'trendline' ? { ...d, extended: !d.extended } : d))} className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-[var(--holo-cyan)] drop-shadow-[0_0_4px_var(--holo-cyan)] text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full transition-colors hover-lift" title="Toggle ray extension"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>Ray Ext</button>
          )}
          <button onClick={deleteSelected} className="flex items-center gap-1.5 bg-[var(--holo-magenta)]/10 hover:bg-[var(--holo-magenta)]/20 border border-[var(--holo-magenta)]/30 text-[var(--holo-magenta)] drop-shadow-[0_0_4px_var(--holo-magenta)] text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full transition-colors hover-lift"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>Destroy</button>
          <button onClick={() => setSelectedId(null)} className="text-[#848e9c] hover:text-white p-1 rounded-full transition-colors"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
      )}
    </>
  );
