...
const StatusBadge: React.FC<{ status: BotStatus | 'connected' | 'disconnected' | 'error' }> = ({ status }) => {
  const map: Record<string, { text: string; cls: string; dot: string }> = {
    ...
    error:        { text: 'FAIL',        cls: 'bg-[#ff073a]/10 text-[#ff073a] border-[#ff073a]/30', dot: 'bg-[#ff073a] animate-pulse' },
    connected:    { text: 'LINKED',      cls: 'bg-[#00f0ff]/10 text-[#00f0ff] border-[#00f0ff]/30', dot: 'bg-[#00f0ff] animate-pulse' },
...
const Field: React.FC<{ label: string; children: React.ReactNode; icon?: any }> = ({ label, children, icon: Icon }) => (
  <div className="relative group">
    <div className="flex items-center gap-1.5 mb-1.5">
      {Icon && <Icon className="w-3 h-3 text-gray-500 group-focus-within:text-[#00f0ff] transition-colors" />}
      <label className="text-[9px] text-gray-400 uppercase tracking-widest font-bold group-focus-within:text-[#00f0ff] transition-colors">{label}</label>
    </div>
    {children}
  </div>
);
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (props) => (
  <input {...props} className={`w-full bg-black/50 border border-white/[0.1] focus:border-[#00f0ff] focus:shadow-[0_0_15px_rgba(0,240,255,0.1)] rounded-lg px-3 py-2 text-white text-[13px] outline-none font-mono font-bold transition-all ${props.className || ''}`} />
);
...
const APModal: React.FC<{ onClose: () => void; onSave: (ap: AccessPoint) => void }> = ({ onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#05070a]/90 backdrop-blur-md flex items-center justify-center p-4 custom-scrollbar overflow-y-auto" onClick={onClose}>
      <div className="bg-[#0b0e11] border border-[#00f0ff]/30 rounded-2xl shadow-[0_0_50px_rgba(0,240,255,0.1)] w-full max-w-md p-6 relative overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="absolute top-0 right-[-10%] w-40 h-40 bg-[#00f0ff]/10 blur-3xl rounded-full" />
        <div className="flex justify-between items-center mb-6 relative z-10 border-b border-white/[0.05] pb-4">
          <div className="flex items-center gap-3">
             <div className="p-2 bg-[#00f0ff]/10 rounded border border-[#00f0ff]/20 text-[#00f0ff]"><Network className="w-5 h-5" /></div>
             <h3 className="text-white font-black tracking-[0.2em] uppercase text-sm">Uplink Gateway</h3>
          </div>
          <button onClick={onClose} className="text-gray-500 hover:text-[#ff073a] transition-colors"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="flex gap-3 mt-6 relative z-10">
          ...
          <button
            ...
            className="flex-1 py-3 bg-[#00f0ff]/10 hover:bg-[#00f0ff]/20 text-[#00f0ff] border border-[#00f0ff]/30 hover:border-[#00f0ff] text-[11px] font-black tracking-widest uppercase rounded box-glow transition-all"
...
const BotModal: React.FC<{ existing?: BotConfig; accessPoints: AccessPoint[]; onClose: () => void; onSave: (bot: BotConfig) => void }> = ({ existing, accessPoints, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#05070a]/90 backdrop-blur-md flex items-center justify-center p-4 overflow-auto py-10" onClick={onClose}>
      <div className="bg-[#0b0e11] border border-[#bc13fe]/30 rounded-2xl shadow-[0_0_50px_rgba(188,19,254,0.1)] w-full max-w-3xl relative overflow-hidden flex flex-col max-h-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-8 overflow-y-auto custom-scrollbar relative z-10 flex-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(['LONG', 'SHORT'] as BotStrategy[]).map(s => (
                    <button key={s} onClick={() => set('strategy', s)}
                      className={`py-2 rounded font-black text-[10px] tracking-widest uppercase transition-all ${cfg.strategy === s ? (s === 'LONG' ? 'bg-[#39ff14]/15 border border-[#39ff14]/50 text-[#39ff14] shadow-[0_0_10px_rgba(57,255,20,0.2)]' : 'bg-[#ff073a]/15 border border-[#ff073a]/50 text-[#ff073a] shadow-[0_0_10px_rgba(255,7,58,0.2)]') : 'bg-black border border-white/[0.1] text-gray-500 hover:text-gray-300'}`}>{s}</button>
                  ))}
                </div>
              </Field>
            </div>
          ...
          {/* Engine Arch */}
          <section className="bg-black/40 border border-white/[0.05] rounded-xl p-4">
            <h4 className="text-[10px] text-[#00f0ff] font-black lowercase tracking-widest mb-4 flex items-center gap-2 font-mono">
               <span className="w-2 h-2 bg-[#00f0ff] animate-pulse" /> [02] engine_architecture
            </h4>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <button key={t.type} onClick={() => set('type', t.type)}
                  className={`p-4 rounded-xl border text-left transition-all ${cfg.type === t.type ? 'bg-[#00f0ff]/10 border-[#00f0ff]/50 shadow-[0_0_15px_rgba(0,240,255,0.1)]' : 'bg-black/50 border-white/[0.05] hover:border-white/[0.2]'}`}>
                  <div className={`text-[12px] font-black tracking-widest uppercase mb-1 flex items-center gap-2 ${cfg.type === t.type ? 'text-[#00f0ff]' : 'text-gray-400'}`}>
          ...
          {/* Trigger */}
          <section className="bg-black/40 border border-white/[0.05] rounded-xl p-4">
             <h4 className="text-[10px] text-[#39ff14] font-black lowercase tracking-widest mb-4 flex items-center gap-2 font-mono">
               <span className="w-2 h-2 bg-[#39ff14] animate-pulse" /> [04] neural_triggers
            </h4>
             <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[ { id: 'NONE', l: 'IMMEDIATE' }, { id: 'MACD', l: 'MACD_CROSS' }, { id: 'RSI', l: 'RSI_EXTREME' }, { id: 'BOLLINGER', l: 'BB_TOUCH' } ].map(sig => (
                <button key={sig.id} onClick={() => set('signal', sig.id as BotSignal)}
                  className={`p-2.5 rounded border text-center font-mono text-[10px] font-black tracking-widest transition-all ${cfg.signal === sig.id ? 'bg-[#39ff14]/15 border-[#39ff14]/50 text-[#39ff14] shadow-[0_0_10px_rgba(57,255,20,0.1)]' : 'bg-black border-white/[0.1] text-gray-600 hover:border-white/[0.3]'}`}>
                  {sig.l}
                </button>
              ))}
            </div>
...
const BotCard: React.FC<{
  ...
  return (
    <div className={`bg-black/60 backdrop-blur-md rounded-xl p-1 relative overflow-hidden transition-all duration-500 ${isRunning ? 'shadow-[0_0_30px_rgba(57,255,20,0.1)] border border-[#39ff14]/30' : 'border border-white/[0.05]'}`}>
      ...
      <div className="bg-[#05070a] rounded-lg h-full p-4 relative z-10 flex flex-col">
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-[9px] font-mono tracking-widest text-[#00f0ff]/70 font-black">
               <Network className="w-3 h-3 text-[#00f0ff]" /> {ap?.name || 'NO_GATEWAY'} <span className="text-gray-600">//</span> {bot.pair}
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => onEdit(bot)} className="p-1.5 text-gray-500 hover:text-[#00f0ff] bg-white/[0.02] border border-white/[0.05] hover:border-[#00f0ff]/30 rounded transition-all"><Settings2 className="w-3.5 h-3.5" /></button>
            <button onClick={() => { if (confirm(`DEAUTHORIZE NODE: ${bot.name}?`)) onDelete(bot.id); }} className="p-1.5 text-gray-500 hover:text-[#ff073a] bg-white/[0.02] border border-white/[0.05] hover:border-[#ff073a]/30 rounded transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <Stat label="NET YIELD" value={`${bot.realizedProfit >= 0 ? '+' : ''}${bot.realizedProfit.toFixed(2)}`} color={bot.realizedProfit > 0 ? '#39ff14' : bot.realizedProfit < 0 ? '#ff073a' : '#848e9c'} sub="USDT" />
          <Stat label="FLOAT PNL" value={`${bot.unrealizedPnl >= 0 ? '+' : ''}${bot.unrealizedPnl.toFixed(2)}`} color={bot.unrealizedPnl > 0 ? '#00f0ff' : bot.unrealizedPnl < 0 ? '#ff073a' : '#848e9c'} sub="USDT" />
        ...
        <div className="mt-auto pt-4 border-t border-white/[0.05]">
          <button
            ...
            className={`w-full py-2.5 rounded font-black text-[11px] tracking-[0.2em] uppercase transition-all flex items-center justify-center gap-2 border group relative overflow-hidden ${
              isRunning ? 'bg-[#ff073a]/10 hover:bg-[#ff073a]/20 border-[#ff073a]/30 text-[#ff073a] box-glow-red' : 'bg-[#39ff14]/10 hover:bg-[#39ff14]/20 border-[#39ff14]/30 text-[#39ff14] box-glow-green'
            }`}
             >
            {isRunning && <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-[#ff073a]/5 to-transparent -translate-x-full group-hover:animate-[shimmer_1s_infinite]" />}
            {!isRunning && <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-[#39ff14]/5 to-transparent -translate-x-full group-hover:animate-[shimmer_1s_infinite]" />}
  ...
  const toggleBot = (id: string) => {
    setBots(prev => prev.map(b => {
      ...
      if (!accessPoints.find(ap => ap.id === b.accessPointId)) {
        toast.error('UPLINK FAILURE: Gateway missing', { style: { background: '#0a0d14', color: '#ff073a', border: '1px solid rgba(255,7,58,0.2)' } });
        return b;
      }
      toast.success(`${b.name} ENGAGED`, { iconTheme: {primary: '#39ff14', secondary: '#0a0d14'}, style: { background: '#0a0d14', color: '#39ff14', border: '1px solid rgba(57,255,20,0.2)' } });
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
             <div className="relative">
               ...
               {runningCount > 0 && <span className="absolute -bottom-1 -right-1 flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#39ff14] opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-[#39ff14] border border-black"></span></span>}
             </div>
             <div>
          ...
          <>
            {bots.length > 0 && (
              <div className="grid grid-cols-3 gap-3 mb-6">
                <Stat label="Total Extracted" value={`${totalPnl >= 0 ? '+' : ''}${totalPnl.toFixed(2)}`} color={totalPnl > 0 ? '#39ff14' : '#eaecef'} sub="USDT_PROFIT" highlight={totalPnl > 0} />
                <Stat label="Engaged Protocols" value={runningCount} color={runningCount > 0 ? '#bc13fe' : '#eaecef'} sub={`/ ${bots.length} TOTAL`} />
                <Stat label="Network Integrity" value="100%" color="#39ff14" sub="OPTIMAL" />
              </div>
            )}
        ...
        {view === 'access' && (
           accessPoints.length === 0 ? (
               <div className="flex flex-col items-center justify-center h-48 border border-white/[0.05] border-dashed rounded-xl bg-black/20 text-center relative overflow-hidden">
                 ...
                 <button onClick={() => setShowAPModal(true)} className="px-6 py-2 border border-[#00f0ff]/50 text-[#00f0ff] text-[10px] tracking-widest uppercase font-black rounded box-glow hover:bg-[#00f0ff]/10 transition-all">Establish Gateway</button>
               </div>
           ) : (
                    ...
                    <div key={ap.id} className="bg-black/40 backdrop-blur-sm border border-white/[0.05] rounded-xl p-4">
                       <div className="flex justify-between items-start mb-4 border-b border-white/[0.05] pb-3">
                         <div>
                            ...
                            <p className="text-[#00f0ff]/60 text-[9px] font-mono tracking-widest mt-1">EXT_NET // {ap.exchange.toUpperCase()}</p>
                         </div>
                         <div className="flex items-center gap-2">
                           <StatusBadge status={ap.status} />
                           <button onClick={() => setAccessPoints(p => p.filter(x => x.id !== ap.id))} className="p-1 hover:bg-white/[0.1] rounded text-gray-500 hover:text-[#ff073a] transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                         </div>
                       </div>
                       ...
                       <div className="space-y-3 font-mono text-[10px]">
                         ...
                         <div className="flex justify-between">
                            ...
                            <span className="text-[#00f0ff] font-black">{bots.filter(b => b.accessPointId === ap.id).length}</span>
                         </div>
                       </div>
                    </div>
                 ))}
