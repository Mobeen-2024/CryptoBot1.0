...
export const SyncTradesModal: React.FC<SyncModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="glass-panel border border-[#2b3139] shadow-2xl rounded-xl w-full max-w-lg overflow-hidden flex flex-col">
        
        {/* Header */}
        <div className="flex justify-between items-center p-4 border-b border-[#2b3139] bg-black/20">
          <div className="flex items-center gap-2">
            <RefreshCw className={`w-5 h-5 text-[var(--holo-cyan)] ${isScanning ? 'animate-spin' : ''}`} />
          ...
          {isScanning ? (
            <div className="flex flex-col items-center justify-center py-10">
              <div className="relative w-16 h-16 mb-6">
                <div className="absolute inset-0 border-4 border-[var(--holo-cyan)]/20 rounded-full"></div>
                <div className="absolute inset-0 border-4 border-[var(--holo-cyan)] rounded-full border-t-transparent animate-spin"></div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center py-10 animate-in zoom-in-95 duration-300">
              <div className="w-16 h-16 bg-[var(--holo-cyan)]/10 rounded-full flex items-center justify-center mb-6">
                <CheckCircle className="w-8 h-8 text-[var(--holo-cyan)]" />
              </div>
            ...
            <div className="flex flex-col animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="max-h-[200px] overflow-y-auto custom-scrollbar">
                  {discrepancies.map((trade) => (
                    <div key={trade.id} className="grid grid-cols-5 gap-4 p-3 border-b border-[#2b3139]/50 last:border-0 hover:bg-white/5 text-xs font-mono items-center transition-colors">
                      ...
                      <div>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${trade.side === 'BUY' ? 'bg-[var(--holo-cyan)]/20 text-[var(--holo-cyan)]' : 'bg-[var(--holo-magenta)]/20 text-[var(--holo-magenta)]'}`}>
                          {trade.side}
                        </span>
                      </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button 
                  ...
                  className="flex-[2] flex items-center justify-center gap-2 py-2.5 bg-[var(--holo-cyan)] hover:bg-[var(--holo-cyan)] text-white rounded-md text-sm font-semibold transition-all hover:shadow-[0_0_20px_rgba(79,70,229,0.4)]"
                >
                  Force Execute Now
